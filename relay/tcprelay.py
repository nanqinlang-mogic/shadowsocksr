...
import sys
sys.path.append("..")
#from . import eventloop, common, lru_cache
import eventloop, common, lru_cache
from common import pre_parse_header, parse_header
from encrypt import encrypt
from obfs import obfs
from service import shell, version
...
class TCPRelayHandler(object):
	...
	def _on_remote_read(self, is_remote_sock):
		...
		try:
			...
			if eventloop.errno_from_exception(e) in (errno.ETIMEDOUT, errno.EAGAIN, errno.EWOULDBLOCK, 10035): #errno.WSAEWOULDBLOCK
				return
		if not data:
			self.destroy()
			return
