...
import sys
sys.path.append("..")
#from . import common, lru_cache, eventloop
...
class DNSResolver(object):
	...
	def __init__(self):
		...
		self._cache = lru_cache.LRUCache(timeout=400)
		self._sock = None
		self._servers = None
		self._parse_resolv()
		self._parse_hosts()
	...
	def _parse_resolv(self):
		self._servers = []
		try:
			with open('dnserver.conf', 'rb') as f:
				content = f.readlines()
				for line in content:
					line = line.strip()
					if line:
		...
		if not self._servers:
			try:
				...
				pass
		if not self._servers:
			self._servers = [('208.67.222.222', 443), ('8.8.8.8', 53)]
	...
	def _call_callback(self, hostname, ip, error=None):
		...
		for callback in callbacks:
			...
			if ip or error:
				...
				callback((hostname, None), Exception('cannot parse hostname %s' % hostname))
	...
	def handle_event(self, sock, fd, event):
		...
		if event & eventloop.POLL_ERR:
			...
			if addr not in self._servers:
				logging.warn('received a packet other than required dns server')
				return
	...
	def _send_req(self, hostname, qtype):
		...
		for server in self._servers:
			logging.debug('resolving %s with type %d + server %s', hostname, qtype, server)
	...
	def resolve(self, hostname, callback):
		...
		if not hostname:
			...
			logging.debug('hosts: %s', hostname)
			ip = self._hosts[hostname]
			callback((hostname, ip), None)
		elif hostname in self._cache:
			logging.debug('cache: %s', hostname)
			...
			if False:
				...
				if addrs:
					...
					logging.debug('DNS resolver %s %s' % (hostname, sa[0]) )
